...
public class SelfCrossing {
	public static void main(String[] args) {
			...
			System.out.println(s.strongPasswordChecker("bbaaaaaaaaaaaaaaaaccccccc"));
		}
	public int strongPasswordChecker(String s)
    {
        ...
        replace=replace - Math.min(deleteCount, oned);

        // deleting 2 chars in (3n+1) repeated chars will save one replacement
        replace=replace - Math.min(Math.max(deleteCount - oned, 0), twod) / 2;

        // deleting 3 chars in (3n+2) repeated chars will save one replacement
        replace=replace - Math.max(deleteCount - oned - twod, 0) / 3;
